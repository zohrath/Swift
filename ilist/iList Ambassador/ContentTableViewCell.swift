...
class ContentTableViewCell: UITableViewCell {
    ...
    func updateWithContent(_ inbox: Inbox) {
        titleLabel.text = inbox.title
        nameLabel.text = inbox.brandName
        if let img = inbox.brandImg, let url = URL(string: img) {
        ...
        if let bg = inbox.brandBg, let url = URL(string: bg) {
