...
// MARK: - Data Model
//
struct RewardList: CreatableFromJSON { // TODO: Rename this struct
    let brand: Brand
    let iconUrl: URL
    let id: Int
    init(brand: Brand, iconUrl: URL, id: Int, title: String) {
        self.brand = brand
        self.iconUrl = iconUrl
        self.id = id
    }
    init?(json: [String: Any]) {
        guard let brand = Brand(json: json, key: "brand") else { return nil }
        guard let iconUrl = URL(json: json, key: "icon_url") else { return nil }
        guard let id = json["id"] as? Int else { return nil }
        guard let title = json["title"] as? String else { return nil }
        self.init(brand: brand, iconUrl: iconUrl, id: id, title: title)
    }
    struct Brand: CreatableFromJSON { // TODO: Rename this struct
        let logotype: URL
        init(logotype: URL) {
            self.logotype = logotype
        }
        init?(json: [String: Any]) {
            guard let logotype = URL(json: json, key: "logotype") else { return nil }
            self.init(logotype: logotype)
        }
    }
}
